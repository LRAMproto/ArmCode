...
GC = load('ByHandCastHit2.mat');

sigfunc = @(A, x)(A(1) ./ (1 + exp(-A(2).*(x+A(3))) ) );
% sigfunc = @(A, x)(A(1) ./ (A(2) + exp(-x)) );
A0 = [40,2,-10]; %// Initial values fed into the iterative algorithm
...
A_fit = A_fit_org.*[1,.89,1.0]+[0,0,-.072]; %GOLDEN
...
%%%%%%%%%%%%%%%%%%%% Graph Comparison %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
A1 = A_fit_org.*[1,.89,1.0]+[0,0,-.072]; %GOLDEN
A2 = A_fit_org.*[1.33,.75,1.04]; % Just kinda thuds
