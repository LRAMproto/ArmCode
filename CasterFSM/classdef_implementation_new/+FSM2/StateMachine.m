classdef StateMachine < hgsetget
    ...
    methods
        ...
        %% DispANSTs(self)
        % Displays information on absolute next-state transitions. Used as
        % a helper function for disp().
        ...
        %% DispNSTs(self)
        % Displays information on next-state transitions. Used as
        % a helper function for disp().
        ...
        % Displays information on error next-state transitions. Used as
        % a helper function for disp().        
        ...
        %% DispInputSymbols(self)
        %
        
        function DispInputSymbols(self)
            fprintf('\tInput Symbols (I):\t');
            if ~(...
                    self.HasDefinedInputs() ||...
                    self.HasDefinedNSTs ||...
                    self.HasDefinedANSTs ||...
                    self.HasDefinedErrorFcns...
                )
                fprintf('None');
            else
        ...
        %% GetInitialNSTStates(self)
        % Gets all initial states of next transition functions.
        ...
        %% GetOutput(self):
        % Returns the output of the state machine.
        
        function result = GetOutput(self)
            result = self.output;
        end
        ...
        function inpts = GetAllInputs(self)
            if ~self.HasDefinedInputs()
                inpts = unique(self.GetUsedInputs());
            else                
            inpts = unique([...
