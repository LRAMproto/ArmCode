...
arm_g_offset = GC.fsData(i,1)-pi;
str_g_offset = mean(GC.fsData(i:i+10,9)) - pi + arm_g_offset;
str_expct = 2*pi - GC.fsData(i:j,1) + str_g_offset; % Expected reading of string angle

%%% String Angle%%%
figure
subplot(2,1,1)
[hA, h_b1, h_b2] = plotyy(GC.fsTime(i:j), [GC.fsData(i:j,9),str_expct],...
...
ylabel(hA(1), 'rad')
ylabel(hA(2), 'rad/sec')
legend('position', 'gravity pos', 'velocity', 'Location', 'SouthEast')
subplot(2,1,2)
plot(GC.fsTime(i:j), (GC.fsData(i:j,9)) + pi-str_expct )
grid on
title('String Angle Relative to gravity')
xlabel('sec')
ylabel('rad')
